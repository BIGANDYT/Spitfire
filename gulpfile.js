...
gulp.task("04-Apply-Xml-Transform", function () {
  var layerPathFilters = ["./src/Framework/**/*.transform", "./src/Domain/**/*.transform", "./src/Project/**/*.transform", "!.src/**/obj/**/App_Config", "!.src/**/bin/**/App_Config"];
...
gulp.task("Publish-Assemblies", function () {
  var root = "./src";
  var binFiles = root + "/**/code/**/bin/Sitecore.{Domain,Framework,Habitat}.*.{dll,pdb}";
...
gulp.task("Auto-Publish-Assemblies", function () {
  ...
  var files = "/**/Sitecore.{Domain,Framework,Habitat}.*.{dll,pdb}";;
  var destination = config.websiteRoot + "/bin/";
  gulp.src(roots, { base: root }).pipe(
    foreach(function (stream, rootFolder) {
      gulp.watch(rootFolder.path + files, function (event) {
